...
> 用来验证把关真的在工作：最终 **放行 67 条、拦下 16 条**。
>
...
83 条原始材料（68 条真实条款 + 15 条抓取残留）→ **放行 67 条，拦下 16 条**：

| 关卡 | 拦什么 | 本项目实测拦下 |
|---|---|---|
| ① 太短碎屑 | 少于 15 字的残片 | **15 条网页抓取残留**（导航菜单 —— 真实脏数据，非构造） |
| ② 完全重复 | 归一化后 md5 相同 | —（真实语料里没有，靠自造数据测） |
| ③ 近似重复 | 余弦相似度 > 0.90 | 《纪律处分规定》第二十五条（与第二十四条太像，见下方"误伤"说明） |
| ④ 个人隐私 | 手机号 / 身份证 / 银行卡 | —（公开制度不含 PII，靠自造数据测） |
| ⑤ 缺少来源/日期 | `source` / `version` 为空 → 出事无法追溯 | —（真实语料都带元信息，靠自造数据测） |
| 版本冲突 | 同一条款有新旧两版 | —（公开制度只有单一版本，靠自造数据测） |
...
> 只能靠"自己造数据"的测试守住（`test_exact_duplicate_rejected` /
> `test_rejected_privacy` / `test_missing_source_rejected` / `test_rejected_old_version`）。
> 不把它们算进"已被真实数据验证"，是这套把关目前的真实覆盖度。
>
> ★ **一个真实的误伤**：《纪律处分规定》第二十五条（考试作弊开除情形）因为和第二十四条
> （作弊留校察看）余弦相似度 > 0.90，被近似重复关拦掉了 —— 可两条都是合法条款。
> 这是"去重"与"别误杀"之间真实的取舍，目前选择保住去重（代价是误伤这 1 条，
> 该条内容在评测集里因此不设为 gold）。
...
| **粗捞池只有 5 条** | `search(k=5)`，而库已有 67 块 | 演示口径是"粗捞 20 精排 3"，代码里是 5。换真实语料后 eval 已经付出代价：q12 因口语化掉出前 5。扩池必须用 eval 数字证明有效，属下一步 |
